...
window.addEventListener('DOMContentLoaded', async function (event) {
  ...
  let moviesfire = querySnapshot.docs
    // Step 1: Construct a URL to get movies playing now from TMDB, fetch
    // data and put the Array of movie Objects in a variable called
    // movies. Write the contents of this array to the JavaScript
    // console to ensure you've got good data
    // ⬇️ ⬇️ ⬇️
    let response = await fetch('https://api.themoviedb.org/3/movie/now_playing?api_key=54ce5b3e7d8077d2525ff43962e9b0a0&language=en-US')
...
// console.log(movies)
    ...
    for (let i=0; i<movies.length; i++) {
        ...
        let docRef = await db.collection('movies').doc(`${movieID}`).get()
